    ...
    <div class="panel container-fluid">
      ...
      <div class="p-0 mb-3 col-md-12">
        ...
        <div>
          <div class="row d-flex justify-content-start align-items-end">
            ...
            >
              <b-form-group :label="entityLabel">
                <b-form-input
                  v-model.trim="concept"
                  :list="allowedEntities.length ? 'lista-entidades' : null"
                  :placeholder="entityPlaceholder"
                  :class="['input_style_b','form-control2', {'is-invalid': !isConceptValid}]"
                />
                <b-form-datalist
                  v-if="allowedEntities.length"
                  id="lista-entidades"
                  :options="allowedEntities"
                  class="listado form-control2"
                />
                <div v-if="!isConceptValid" class="invalid-feedback d-block">
                  Debes seleccionar una entidad válida de la lista.
                </div>
              </b-form-group>
            </div>
          ...
          <div
            v-if="!isPagaduriaValid || !isMonthValid || !isYearValid || !isEntidadDemandanteValid || !isConceptValid"
            class="text-danger"
          >
            ...
            <div v-if="!isConceptValid && showConceptValidation">La entidad es obligatoria y debe estar en la lista.</div>
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        incapacidades: false,
        allowedEntities: [],
        entityRestrictionsLoaded: false
      }
    },
    computed: {
      showConceptValidation () {
        return this.selectedEstado === 'Al día' || this.selectedEstado === 'Todas'
      },
      isConceptValid () {
        if (!this.showConceptValidation) return true
        if (!this.allowedEntities.length) return true
        const val = (this.concept || '').trim().toUpperCase()
        return this.allowedEntities.map(e => e.toUpperCase()).includes(val)
      },
      entityLabel () {
        return this.allowedEntities.length ? 'Entidad (selecciona de la lista)' : 'Entidad (Banco o Financiera)'
      },
      entityPlaceholder () {
        return this.allowedEntities.length ? 'Escribe y selecciona una entidad permitida' : 'Entidad'
      }
    },
    watch: {
    ...
    methods: {
      ...
      async getEntityRestrictions () {
        try {
          const { data } = await axios.get('/empresas/entidad-filtros')
          this.allowedEntities = Array.isArray(data) ? data.filter(Boolean) : []
        } catch (e) {
          this.allowedEntities = []
        } finally {
          this.entityRestrictionsLoaded = true
        }
      },
      validarEntidadDemandante () {
        this.isEntidadDemandanteValid =
          !(this.selectedEstado === 'Embargado' || this.selectedEstado === 'Todas') ||
          !!(this.entidadDemandante && this.entidadDemandante.trim())
      },
      validateInputs () {
        const m = String(this.month).trim()
        const y = String(this.year).trim()
      ...
      async search (page = 1) {
        if (!this.entityRestrictionsLoaded) await this.getEntityRestrictions()
