<template>
    <div>
      ...
      <div class="mb-2 mt-5"><h3 class="heading-title">Resultados de la consulta (Cartera en Mora)</h3></div>
      ...
      <div class="table-responsive">
        <b-table
          ...
          <template v-slot:cell(actions)="{ item }">
            <b-button variant="primary" @click="$emit('ver-causales', item.doc, item.id)">
              Ver
            </b-button>
          </template>
        </b-table>
  ...
  export default {
    ...
    computed: {
      filtrados () {
        ...
        return slice.map(i => ({ ...i, valor: this.formatCurrency(i.valor) }))
      },
      totalCuotas () {
        const n = (this.items || []).reduce((t, i) => t + this.parseNumber(i.valor), 0)
