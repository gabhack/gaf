            ...
            <b-form-group label="Departamento" label-for="empresa_departamento">
              ...
              >
                ...
                <b-form-select-option
                  v-for="dep in ubicaciones.departamentos"
                  :key="dep.id"
                  :value="dep.id"
                >
                  {{ dep.nombre }}
          ...
          <b-col cols="4">
            <b-form-group label="Ciudad" label-for="empresa_ciudad">
              ...
              >
                ...
                <b-form-select-option
                  v-for="ciudad in ubicaciones.ciudades"
                  :key="ciudad.id"
                  :value="ciudad.id"
                >
                  {{ ciudad.nombre }}
  ...
  export default {
    ...
    methods: {
      ...
      async cargarUbicaciones() {
        ...
        try {
          let depResponse = await axios.get('/json/departamentos.json');
          this.ubicaciones.departamentos = depResponse.data;
        } catch (error) {
          console.error('Error al cargar departamentos:', error);
        }
        // Cargar municipios desde el JSON local
        try {
          let munResponse = await axios.get('/json/municipios.json');
      ...
      filtrarMunicipios() {
        if (this.form.empresa.departamento_id) {
          this.ubicaciones.ciudades = this.municipiosData.filter(
            municipio => municipio.departamento_id === this.form.empresa.departamento_id
          );
        } else {
