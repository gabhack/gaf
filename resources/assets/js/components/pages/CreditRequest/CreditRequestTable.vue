          ...
          <td>
            ...
            <template v-if="credit.documents && credit.documents.length">
              <div v-for="(doc, i) in credit.documents" :key="doc.id">
                <a :href="getDownloadUrl(doc.file_path)" target="_blank">doc-{{ i + 1 }}</a>
              </div>
            </template>
            <span v-if="!credit.pdf_path && (!credit.documents || !credit.documents.length)">No hay documentos</span>
          </td>
          <td v-if="isAdmin">
...
export default {
  ...
  methods: {
    ...
    getDownloadUrl (p) {
      return `/storage/${String(p || '').replace('public/', '')}`
    },
...
.status-badge{ display:inline-flex; align-items:center; gap:6px; padding:4px 14px; border-radius:9999px; font-size:.9rem; font-weight:600; }
