    ...
    <div style="padding: 30px">
        ...
        <div v-if="results.length" class="panel mb-3 col-md-12">
            ...
            <div class="panel-body">
                ...
                <!-- Campo para filtrar por Entidad (cupones, embargos, descuentos) -->
                <b-form-group>
                    <b-form-input
                        v-model="entityFilter"
                        placeholder="Buscar por entidad (cupón, embargo o descuento)"
                        class="input_style_b form-control2"
                    ></b-form-input>
                </b-form-group>
...
export default {
    ...
    computed: {
        filteredResults() {
            ...
            const entityTerm = this.entityFilter.toLowerCase()

            return this.results
                .filter(r => {
                    const docMatches = searchTerm.length < 3 || r.doc.toString().includes(searchTerm)
                    if (!docMatches) return false

                    if (!entityTerm) return true

                    const matchCupones =
                        Array.isArray(r.cupones) &&
                        r.cupones.some(c => c.concept && c.concept.toLowerCase().includes(entityTerm))
                    const matchEmbargos =
                        Array.isArray(r.embargos) &&
                        r.embargos.some(e => e.entidaddeman && e.entidaddeman.toLowerCase().includes(entityTerm))
                    const matchDescuentos =
                        Array.isArray(r.descuentos) &&
                        r.descuentos.some(d => d.mliquid && d.mliquid.toLowerCase().includes(entityTerm))

                    return matchCupones || matchEmbargos || matchDescuentos
                })
                .map(r => {
                    if (!entityTerm) return r
                    const newR = { ...r }

                    if (Array.isArray(newR.cupones)) {
                        newR.cupones = newR.cupones.filter(
                            c => c.concept && c.concept.toLowerCase().includes(entityTerm)
                        )
                    }
                    if (Array.isArray(newR.embargos)) {
                        newR.embargos = newR.embargos.filter(
                            e => e.entidaddeman && e.entidaddeman.toLowerCase().includes(entityTerm)
                        )
                    }
                    if (Array.isArray(newR.descuentos)) {
                        newR.descuentos = newR.descuentos.filter(
                            d => d.mliquid && d.mliquid.toLowerCase().includes(entityTerm)
                        )
                    }
                    return newR
                })
