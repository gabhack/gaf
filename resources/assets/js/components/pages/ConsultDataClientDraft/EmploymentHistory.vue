<template>
        ...
        <b-collapse id="info-historial" v-model="visible" class="mt-2">
                  ...
                  <table role="table" aria-colcount="1" class="table b-table table-striped table-hover">
                    ...
                    <tbody role="rowgroup">
                      <tr role="row" v-for="(item, idx) in datamesSedArray" :key="idx">
                        <td role="cell" aria-colindex="1">{{ item.fechaVinculacion || 'N/A' }}</td>
                      </tr>
                      <tr role="row" v-if="!datamesSedArray.length && fechavinc.vinc">
                        <td role="cell" aria-colindex="1">{{ fechavinc.vinc }}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
          ...
          <template v-if="pagaduriaType === 'FOPEP'">
              ...
              <div class="col-12 col-48 px-0">
                <div class="mt-3 table-responsive">
                  <table role="table" aria-colcount="1" class="table b-table table-striped table-hover">
                    ...
                    <tbody role="rowgroup">
                      <tr role="row">
                        <td aria-colindex="1" role="cell">{{ valorIngresoUi | currency }}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
            ...
            <div class="col-4" v-if="ingresosExtras.length > 0">
              ...
              <div class="row">
                <div class="col-2"><b class="panel-label">CONCEPTO:</b></div>
                <div class="col-2"><b class="panel-label">VALOR:</b></div>
              </div>
              <div class="row" v-for="extra in ingresosExtras" :key="extra.code">
                <div class="col-2"><p class="panel-value">{{ extra.concept }}</p></div>
                <div class="col-2"><p class="panel-value">{{ extra.ingresos | currency }}</p></div>
              </div>
            </div>
  
            <div class="col-2">
              <b class="panel-label">TIPO PENSION:</b>
              <div><p class="panel-value">{{ datamesSed.tp || 'N/A' }}</p></div>
            </div>
            <div class="col-md-2">
              <b class="panel-label">VALOR INGRESO:</b>
              <div><p class="panel-value">{{ datamesSed.vpension | currency }}</p></div>
            </div>
            <div class="col-md-2">
              <b class="panel-label">VALOR SALUD:</b>
              <div><p class="panel-value">{{ datamesSed.vsalud | currency }}</p></div>
            </div>
            <div class="col-md-2">
              <b class="panel-label">VALOR DESCUENTOS:</b>
              <div><p class="panel-value">{{ datamesSed.vdesc | currency }}</p></div>
            </div>
            <div class="col-md-2">
              <b class="panel-label">VALOR CUPO:</b>
              <div><p class="panel-value">{{ datamesSed.cupo | currency }}</p></div>
            </div>
            <div class="col-md-2">
              <b class="panel-label">VALOR EMBARGOS:</b>
              <div><p class="panel-value">{{ datamesSed.vembargos | currency }}</p></div>
            </div>
          </template>
          ...
          <div v-if="datamessedvalle">
            <div class="col-12">
              <div class="mt-3 table-responsive">
                <table role="table" aria-colcount="2" class="table b-table table-striped table-hover">
                  <thead role="rowgroup" class="table-header-nowrap">
                    <tr role="row">
                      <th role="columnheader" scope="col" aria-colindex="1" colspan="1">VALOR INGRESO:</th>
                      <th role="columnheader" scope="col" aria-colindex="2" colspan="1"><b class="panel-label">SUELDO BASICO:</b></th>
                    </tr>
                  </thead>
                  <tbody role="rowgroup">
                    <tr role="row">
                      <td aria-colindex="1" role="cell" colspan="1">
                        <p class="panel-value">{{ valorIngresoUi | currency }}</p>
                      </td>
            ...
            <div class="col-2">
              ...
              <div><p class="panel-value">{{ datamessedvalle.fechingr || 'N/A' }}</p></div>
            </div>
            <div class="col-2">
              <b class="panel-label">AREA DE DESEMPEÑO:</b>
              <div><p class="panel-value">{{ datamessedvalle.esquema || 'N/A' }}</p></div>
            </div>
            <div class="col-2">
              <b class="panel-label">CARGO:</b>
              <div><p class="panel-value">{{ datamessedvalle.cargo || 'N/A' }}</p></div>
            </div>
            <div class="col-2">
              <b class="panel-label">FECHA VINCULACIÓN:</b>
              <div><p class="panel-value">{{ datamessedvalle.fecnombr || 'N/A' }}</p></div>
            </div>
            <div class="col-2">
              <b class="panel-label"> TIPO DE CONTRATO:</b>
              <div><p class="panel-value">{{ datamessedvalle.ncontr || 'N/A' }}</p></div>
            </div>
            <div class="col-2">
              <b class="panel-label">GRADO:</b>
              <div><p class="panel-value">{{ datamessedvalle.grado ? datamessedvalle.grado : 'N/A' }}</p></div>
            </div>
            <div class="col-2">
              <b class="panel-label">PRINCIPAL :</b>
              <div><p class="panel-value">{{ datamessedvalle.dependencia ? datamessedvalle.dependencia : 'N/A' }}</p></div>
            </div>
            <div class="col-2">
              <b class="panel-label">SEDE:</b>
              <div><p class="panel-value">{{ datamessedvalle.centrocosto ? datamessedvalle.centrocosto : 'N/A' }}</p></div>
            </div>
            <div class="col-2">
              <b class="panel-label">TIPO VINCULACIÓN:</b>
              <div><p class="panel-value">{{ datamessedvalle.nivcontr || 'N/A' }}</p></div>
            </div>
            <div class="col-2">
              <b class="panel-label">ESTADO LABORAL:</b>
              <div><p class="panel-value">{{ datamessedvalle.estlaboral || 'N/A' }}</p></div>
            </div>
            <div class="col-2">
              <b class="panel-label">CENTRO DE EDUCACIÓN:</b>
              <div><p class="panel-value">{{ datamessedvalle.centrocosto || 'N/A' }}</p></div>
            </div>
            <div class="col-2">
              <b class="panel-label">SEDE EN LA QUE PRESTA EL SERVICIO:</b>
              <div><p class="panel-value">{{ datamessedvalle.sedecoleg || 'N/A' }}</p></div>
            </div>
            <div class="col-2">
              <b class="panel-label">CIUDAD LABORAL:</b>
              <div><p class="panel-value">{{ datamessedvalle.ciudad ? datamessedvalle.ciudad : 'N/A' }}</p></div>
            </div>
          </div>
          ...
          <template v-if="datamesfidu">
            <div class="col-2">
              <b class="panel-label">VALOR INGRESO:</b>
              <div><p class="panel-value">{{ datamesfidu.vpension | currency }}</p></div>
            </div>
            <div class="col-2">
              <b class="panel-label">VINCULACION:</b>
              <div><p class="panel-value">{{ datamesfidu.vinc || 'N/A' }}</p></div>
            </div>
            <div class="col-2">
              <b class="panel-label">FECHA DE PAGO PENSION:</b>
              <div><p class="panel-value">{{ datamesfidu.fechpago || 'N/A' }}</p></div>
            </div>
            <div class="col-2">
              <b class="panel-label">VALOR DESCUENTO:</b>
              <div><p class="panel-value">{{ datamesfidu.vdescbruto | currency }}</p></div>
            </div>
          </template>
            ...
            <div class="col-12 col-48 px-0">
              <div class="mt-3 table-responsive">
                <table role="table" aria-colcount="2" class="table b-table table-striped table-hover">
                  <thead role="rowgroup" class="table-header-nowrap">
                    <tr role="row">
                      <th role="columnheader" scope="col" aria-colindex="1" colspan="1">Valor de Ingreso</th>
                    </tr>
                  </thead>
                  <tbody role="rowgroup">
                    <tr role="row">
                      <td aria-colindex="1" role="cell" colspan="1">
                        {{ valorIngresoUi | currency }}
                      </td>
                    </tr>
                  </tbody>
                </table>
          ...
          <div v-if="ingresosExtras.length > 0" style="width: 100%">
            <div class="mt-3 table-responsive">
              <table role="table" aria-colcount="2" class="table b-table table-striped table-hover">
                <thead role="rowgroup" class="table-header-nowrap">
                  <tr role="row">
                    <th role="columnheader" scope="col" aria-colindex="1" colspan="2">Ingresos Extras</th>
                  </tr>
                </thead>
                <tbody role="rowgroup">
                  <tr role="row" v-for="(item, index) in ingresosExtras" :key="index">
                    <td colspan="1">{{ item.concept }}</td>
                    <td colspan="1">{{ item.ingresos | currency }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div v-if="arrayCoupons.length > 0" style="width: 100%">
            <div class="mt-3 table-responsive">
              <table role="table" aria-colcount="2" class="table b-table table-hover">
                <thead role="rowgroup" class="table-header-nowrap">
                  <tr role="row">
                    <th class="text-left" role="columnheader" scope="col" aria-colindex="2" colspan="2">Días Laborados</th>
                  </tr>
                </thead>
                <tbody role="rowgroup">
                  <tr role="row">
          ...
          <div class="col-6" v-if="fechavinc.vinc">
            ...
            <div><p class="panel-value">{{ fechavinc.vinc || 'N/A' }}</p></div>
          </div>
        </b-collapse>
      </div>
    </div>
  ...
  export default {
    ...
    data () {
      return {
        Sueldo: [{ key: 'concept', label: 'Sueldo básico' }, { key: 'ingresos', label: 'Valor' }],
        Extras: [{ key: 'concept', label: 'Ingresos extras' }, { key: 'ingresos', label: 'Valor' }],
        Ingreso: [{ key: 'label', label: 'Valor ingreso' }, { key: 'value', label: 'Valor' }],
        fields: [
          { key: 'fecha_ingreso', label: 'Fecha ingreso', sortable: false, formatter: v => v || 'N/A' },
          { key: 'cargo', label: 'Cargo', sortable: false, formatter: v => v || 'N/A' },
          { key: 'grado', label: 'Grado', sortable: false, formatter: v => v || 'N/A' },
          { key: 'depen', label: 'Principal', sortable: false, formatter: v => v || 'N/A' },
          { key: 'ciudad', label: 'Ciudad laboral', sortable: false, formatter: v => v || 'N/A' }
        ],
        arrayCoupons: [],
        visible: true
      }
    ...
    computed: {
      ...
      ...mapState('pagaduriasModule', ['pagaduriaType', 'selectedPeriod']),
      ...
      datamesSedArray () {
        return this.datamesSed ? [this.datamesSed] : []
      },
      valorIngresoUi () {
        const getterVal = Number(this.valorIngreso || 0)
        const basicoRaw = this.datamesSed && this.datamesSed.basico ? String(this.datamesSed.basico) : ''
        const basico = basicoRaw ? Number(basicoRaw.replace(/\D/g, '')) : 0
        return getterVal > 0 ? getterVal : basico
      }
    },
    methods: {
  ...
  li { list-style-type: none; margin-bottom: 10px; padding: 0; border-bottom: none; }
  p { margin: 0; font-size: 14px; }
  b { font-size: 14px; }
  .table thead th { font-weight: 600; vertical-align: middle; }
  @media (min-width: 770px) { .col-48 { max-width: 48%; } }
  </style>
