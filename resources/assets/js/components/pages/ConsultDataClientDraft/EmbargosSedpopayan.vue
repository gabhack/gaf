    ...
    <div class="panel panel-primary mb-3">
      ...
      <!-- <div class="panel-body">
        <div class="row">
          <div :class="label.colClass || 'col-2'" v-for="label in labels" :key="label.field">
            ...
            <template v-if="embargossedpopayan.length > 0">
              <div v-for="(embargossedpopayan, key) in embargossedpopayan" :key="key">
                <p class="panel-value">
                  <template v-if="embargossedpopayan[label.field]">
                    {{ embargossedpopayan[label.field] }}
                  </template>
                  <template v-else-if="embargossedpopayan[label.field] || label.currency">
                    {{ embargossedpopayan[label.field] | currency }}
                  </template>
      ...
      <div class="panel-body">
        <div class="row">
          ...
          <div class="col-2">
            <b class="panel-label table-text">CUENTA:</b>
          </div>
          <div class="col-1">
            <b class="panel-label table-text">CUOTA DEUDA:</b>
          </div>
          <div class="col-2">
            <b class="panel-label table-text">JUZGADO:</b>
          </div>
          <div class="col-2">
            <b class="panel-label table-text">NOMBRE ENTIDAD CEDIENTE:</b>
          </div>
          <div class="col-2">
            <b class="panel-label table-text">EXPEDIENTE:</b>
          </div>
        </div>
        ...
        <div v-for="(item, key) in embargossedpopayan" :key="key" class="row panel-br-light-green pt-3">
          ...
          <div class="col-2 px-0">
            <p>{{ item.ndem ? item.ndem : '-' }}</p>
          </div>

          <div class="col-2 px-0">
            <p>{{ item.cuenta ? item.cuenta : '-' }}</p>
          </div>
          ...
          <div class="col-2">
            <p>{{ item.expediente ? item.expediente : '-' }}</p>
          </div>
        </div>
      </div>
    </div>
...
<script>
export default {
  name: 'EmbargosSedpopayan',
  props: ['embargossedpopayan'],
  data() {
    return {
      labels: [
